...
const AnimeCard = ({ id, title, coverImage, format, description, genres, episodes, studios }: AnimeCardProps) => {
  ...
  const filteredStudio = studios.nodes.filter((studio) => studio.isAnimationStudio) // Filtrar los estudios dejando los de animación
  ...
  return (
    ...
    >
      ...
      <section className="relative">
        ...
        >
          <p className="bg-secondary font-medium text-accent text-sm px-2 py-0.5 rounded-sm">
            {formats[format]}
          </p>
        </div>
      </section>
      ...
      <div>
        <p className="font-semibold text-white/90 line-clamp-2 group-hover:text-accent-light">
          {title.romaji}
        </p>
      </div>
    </div>
